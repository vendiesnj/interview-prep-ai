...
const COACHING: Record<DimensionKey, { strong: string; good: string; moderate: string; weak: string; critical: string }> = {
  narrative_clarity: {
    ...
    good: "The narrative holds together well. A slightly tighter opening would make the through-line even cleaner.",
    moderate: "The story has the right ingredients but needs a cleaner through-line. Try narrating it in one sentence before you start — 'I did X which led to Y' — then build outward.",
    ...
    critical: "The answer doesn't yet tell a coherent story. Write S-T-A-R on paper, say 'Situation:' out loud, and work through each section. The ideas are there — they need sequencing.",
  },
  evidence_quality: {
    strong: "Concrete, specific, and credible — the proof points make this answer stand out from generic responses.",
    good: "Good specificity overall. One more metric or named tool in your Result would push this to excellent.",
    ...
    weak: "The answer describes the shape of the work but not the substance. Name one specific metric, constraint, or named tool. 'Reduced reporting time' is weak; 'cut weekly reporting from 6 hours to 45 minutes' is strong.",
    critical: "Without concrete proof points, this answer can't score above average. Every strong interview answer has at least one named number, tool, or observable change. Add that before your next attempt.",
  },
  ownership_agency: {
    strong: "Strong first-person ownership — the interviewer clearly understands what you personally drove.",
    good: "Clear personal ownership. Watch for any 'we' slippage in your result section — that's where credit often diffuses.",
    moderate: "Ownership is present but soft in places. Replace 'we worked on' with 'I led' and 'I drove' — the difference in how it's received is significant.",
    weak: "The answer distributes credit too broadly. Start your next answer with 'I decided…' or 'I owned…' — that single change reshapes how the whole story lands.",
  ...
  vocal_engagement: {
    ...
    good: "Good vocal variety overall. Add a slight pitch lift specifically at your result sentence — that's the moment that deserves the most emphasis.",
    ...
    weak: "Flat delivery is obscuring good content. When you reach your result, pause for a beat and let your voice lift slightly — interviewers decide if they heard something important based on vocal emphasis.",
    critical: "Both pitch and energy are flat throughout. Start smaller: in your next attempt, just change how you deliver the final sentence. Slow it down 30% and add emphasis. That's the only change needed right now.",
  },
  response_control: {
    strong: "Clean, controlled delivery — pacing and fluency are working in your favor.",
    good: "Good control overall. Watch for any filler clusters at transitions — those are where most disfluency occurs under pressure.",
    moderate: "Pacing and fluency have some rough patches. Deliberate pauses at transitions (between S→T, T→A, A→R) eliminate fillers better than any other technique.",
    weak: "Filler words and pacing are interfering with the content. Replace every 'um' or 'uh' with a one-beat pause. Silence sounds more confident than a filler — and it gives you a moment to think.",
    critical: "Delivery control is the main barrier right now. Before your next attempt, record yourself saying just the result sentence — no fillers, controlled pace. That single sentence sets the tone for the whole answer.",
  },
  cognitive_depth: {
    strong: "The answer demonstrates sophisticated thinking — tradeoffs, nuance, and clear reasoning all present.",
    good: "Good analytical depth. One more 'on the other hand' or acknowledged tradeoff would signal senior-level thinking.",
    moderate: "The answer shows competence but not yet analytical depth. Add one line that acknowledges a tradeoff or constraint you navigated: 'The challenge was that X, so I chose Y over Z because…'",
    weak: "The answer lacks the reasoning depth that separates strong candidates. Interviewers at senior levels specifically look for: what you considered, why you chose one path, what the tradeoff was. Add one of those.",
  ...
  presence_confidence: {
    ...
    good: "Good presence overall. Stay on camera during the result sentence — that's the moment eye contact matters most.",
    ...
    weak: "Confidence signals are mixed. On video, where your eyes go shapes the impression more than your words. Hold the camera gaze through every key claim — especially when you're uncertain.",
    critical: "Low presence and confidence signals are working against strong content. On your next attempt, commit to sustained eye contact from your first sentence. It will feel uncomfortable — that discomfort is the signal improving.",
  },
};
