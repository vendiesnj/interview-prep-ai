...
async def analyze(audio: UploadFile = File(...)):
    ...
    Returns acoustic metrics computed from waveform + downsampled time series.
    """
    ...
    # Preserve extension if possible (helps decoders); default to .webm
    suffix = ".webm"
    ...
    # ----------------------------
    # MONOTONE score (0-10) (lower is better)
    # ----------------------------
    if voiced_f0.size == 0:
        monotone_score = 10.0
    else:
    ...
    # Align pitch series length to energy series length
    if pitch_series.shape[0] != energy_series.shape[0]:
        x_old = (
            np.linspace(0.0, 1.0, num=pitch_series.shape[0])
            if pitch_series.shape[0] > 1
            else np.array([0.0])
        )
    ...
    t_series = librosa.frames_to_time(frames, sr=sr, hop_length=hop)
