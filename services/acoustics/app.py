...
async def analyze(audio: UploadFile = File(...)):
    ...
    # Tempo dynamics (0-10) based on onset strength variation
    try:
        if onset_env is None:
            onset_env = librosa.onset.onset_strength(y=y, sr=sr)

        if float(np.mean(onset_env)) <= 1e-8:
            tempo_dynamics = 0.0
        else:
            onset_cv = float(np.std(onset_env) / np.mean(onset_env))
            tempo_dynamics = float(round(min(max((onset_cv - 0.25) / 1.25, 0.0), 1.0) * 10.0, 2))
    except Exception:
        tempo_dynamics = 0.0

    # ----------------------------
    # SERIES (downsampled for UI)
    # ----------------------------
    energy_series = rms
